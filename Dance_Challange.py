# -*- coding: utf-8 -*-
"""
Created on Fri May 13 22:04:50 2022
...
def draw():
    global game_over, score, say_dance, dance_length, score_1, score_2
    ...
    if not game_over:
        ...
        right.draw()
        left.draw()
        screen.draw.text("Level: " +str(dance_length - 3), color="black",topleft=(380, 10))
        screen.draw.text("Player 1: " +str(score_1), color="black",topleft=(10, 25))
        screen.draw.text("Player 2: " +str(score_2), color="black",topleft=(650, 25))
        if say_dance:
...
def generate_moves():
    global move_list, dance_length, count, levels #added levels
...
def on_key_up(key):
    global score_1, score_2, game_over, move_list, current_move
    if key == keys.UP:
        update_dancer(0)
...
#change keys to W-A-S-D for second player
    if key == keys.W:
        update_dancer(0)
        if move_list[current_move] == 0:
            score_2 = score_2 + 1
    ...
    return
generate_moves()
music.play("music_duck") #CHANGED MUSIC
def update():
    global game_over, current_move, moves_complete, levels, dance_length, score_1, score_2
    if not game_over:
        if moves_complete:
            if (levels %2 ==0):
                dance_length += 1
